...
object ArchiveService {
    ...
    suspend fun exportMonth(context:Context,db:AppDb,year:Int,month:Int):ExportResult = withContext(Dispatchers.IO) {
        ...
        ZipOutputStream(BufferedOutputStream(FileOutputStream(zip))).use { z ->
            ...
            val csv=buildString {
                appendLine("id,관리번호,의뢰일,조사담당자,담당자전화,담당자Fax,채무자,완료요청일,조사예정일,방문순서,물건소재지,소유자주소,기본주소구분,직접입력주소,대출종류,영업점,영업점전화,영업점Fax,조사의뢰자,상태,조사시작시간,조사완료시간,비고")
                cases.forEach{c->
                    appendLine(
                        listOf(
                            c.id,c.managementNo,c.requestDate,c.investigator,c.investigatorPhone,c.investigatorFax,
                            c.debtorName,c.dueDate,c.plannedDate,c.routeOrder,c.propertyAddress,c.ownerAddress,c.defaultAddressType,c.customMapAddress,c.loanType,c.branch,c.branchPhone,c.branchFax,c.requester,

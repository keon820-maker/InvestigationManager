...
object OriginalFileStore {
    ...
    fun finalizeCamera(file:File,caseId:Long,type:String):Saved {
        val finalDir=File(file.parentFile?.parentFile?.parentFile,"$caseId").apply{mkdirs()}
        val final=File(finalDir,"${type.lowercase()}_${System.currentTimeMillis()}.jpg")
        file.copyTo(final, overwrite=true); file.delete()
    ...
    private fun buildAttachment(file:File,caseId:Long,type:String,mime:String):Attachment {
        ...
        return Attachment(caseId=caseId,type=type,originalName=file.name,localPath=file.absolutePath,mimeType=mime,
            byteSize=file.length(),width=opts.outWidth.takeIf{it>0},height=opts.outHeight.takeIf{it>0},capturedAt=dt,sha256=sha256(file))
    }

    fun sha256(file:File):String {
        val md=MessageDigest.getInstance("SHA-256")
        FileInputStream(file).use { input -> val b=ByteArray(1024*1024); while(true){ val n=input.read(b); if(n<0)break; md.update(b,0,n)} }

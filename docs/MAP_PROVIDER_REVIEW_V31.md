...
v0.31에서는 OSM 지도를 유지하고 태블릿 레이아웃 경계와 폭 조절을 먼저 해결했다. v0.32에서 화면 지도를 Kakao Maps SDK v2로 교체하고, 영구 APK 서명키의 해시를 카카오 Android 플랫폼에 등록했다.

현재 주소→좌표 변환은 Android `Geocoder`를 유지한다. 카카오 Local 주소검색을 추가할 때는 같은 실제 조사 주소 표본으로 결과를 대조한 뒤 전환한다.
